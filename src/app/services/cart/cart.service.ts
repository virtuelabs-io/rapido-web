...
export class CartService extends RapidoHttpService<CartItem> {
  ...
  getCartItems() {
    return new Promise(resolve=>{
      let cartItemsObject;
      this.getList([Constants.CART_APIS.api, 'items'].join("/"),this.addAuthHeader(this.initializeHeaders()))
        .subscribe(data => {
          if (data.length > 0){
      ...
      this.getList([Constants.CART_APIS.api, 'in-cart-items'].join("/"),this.addAuthHeader(this.initializeHeaders()))
        .subscribe(data => {
          if (data.length > 0){
            ...
            resolve ({ error: "No data found found for the items in cart(if they exist)" })
          }
        })
    });
  }
  ...
  getSavedForLaterCartItems(){
    return new Promise(resolve=>{
      let cartItemsObject;
      this.getList([Constants.CART_APIS.api, 'saved-for-later-items'].join("/"),this.addAuthHeader(this.initializeHeaders()))
        .subscribe(data => {
          if (data.length > 0){
            cartItemsObject = this.formatCartItems(data)
            this.getProductDetails(data)
            ...
            resolve ({ error: "No data found found for the items saved for later (if they exist)" })
          }
        })
    });
  }
  ...
  prepareCartItemDetailsList(productDetails: any, cartItemsObject: any): Array<CartItemDetails>{
    let cartItemDetailsList: CartItemDetails[] = []
    for(let product in productDetails.hits.hit){
      cartItemDetailsList.push(new CartItemDetails(

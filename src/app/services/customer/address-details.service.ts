...
export class AddressDetailsService extends RapidoHttpService<AddressDetails>{
  ...
  getAddressDetails(id: number){
    return this.get([Constants.CUSTOMER_APIS.address.api,String(id)].join("/"),this.addAuthHeader(this.initializeHeaders()))
  }

  postAddressDetails(addressDetails: AddressDetails){
    return this.post(Constants.CUSTOMER_APIS.address.api, addressDetails, this.addAuthHeader(this.initializeHeaders()))
  }

  putAddressDetails(addressDetails: AddressDetails){
    return this.put([Constants.CUSTOMER_APIS.address.api,String(addressDetails.id)].join("/"), addressDetails, this.addAuthHeader(this.initializeHeaders()))
  }

  deleteAddressDetails(id: number){
    return this.delete([Constants.CUSTOMER_APIS.address.api,String(id)].join("/"),this.addAuthHeader(this.initializeHeaders()))
  }
}

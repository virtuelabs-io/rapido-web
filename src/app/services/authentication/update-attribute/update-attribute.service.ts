...
export class UpdateAttributeService extends AuthenticationService {
  ...
  updateAttributes(){
    return new Promise( (resolve, reject) => {
      this._userProfile.cognitoUser.updateAttributes(this._attributeList, function(err, result) {
        if(err){
          reject(new Response( 1, err.message, err ))
        }
        resolve( new Response( 0, Constants.SUCCESSFULLY_UPDATED_ATTRIBUTES, result ))
      });
    })
  }
}

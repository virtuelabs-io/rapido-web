...
describe('ProductsService', () => {
  ...
  it(
    'Request to mandatory query fields',
    fakeAsync(
      inject(
        ...
        (service: ProductsService, backend: HttpTestingController) => {
          ...
          const requestWrapper = backend.expectOne({url: Constants.environment.productSearchEndPoint + "q=watch&size=1"});
    ...
    fakeAsync(
      inject(
        ...
        (service: ProductsService, backend: HttpTestingController) => {
          ...
          const requestWrapper = backend.expectOne({url: Constants.environment.productSearchEndPoint + "q=watch&size=1&cursor=initial&start=1&sort=_score desc"});
          requestWrapper.flush(responseObject);

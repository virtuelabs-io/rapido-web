...
export class GuestCartService extends RapidoHttpService<GuestCartItem> {
      ...
      this.getList([Constants.GUESTS_APIS.api, localStorage.getItem(Constants.RAPIDO_SESSION_ID), 'cart', 'items'].join("/"))
        .subscribe(data => {
          if (data.length > 0){
            ...
            resolve ({ error: "No data found found for all items in cart(if they exist)" })
          }
        })
    });
  }
  ...
  postGuestCartItemList(guestCartItems: GuestCartItem[]){
    return this.postList([Constants.GUESTS_APIS.api, localStorage.getItem(Constants.RAPIDO_SESSION_ID), 'cart', 'items'].join("/"), guestCartItems)
  }

  deleteGuestCartItem(guestCartItem: GuestCartItem){
    return this.put([Constants.GUESTS_APIS.api, localStorage.getItem(Constants.RAPIDO_SESSION_ID), 'cart', 'item'].join("/"), guestCartItem)
  }

  deleteGuestCartItems(){
    return this.delete([Constants.GUESTS_APIS.api, localStorage.getItem(Constants.RAPIDO_SESSION_ID), 'cart', 'items'].join("/"))
  }
  ...
  getProductDetails(cartItems: GuestCartItem[]){
    ...
    for(let _item in cartItems){
      _items.push(Constants.SEARCH_QUERY.term.replace("$", String(cartItems[_item].product_id)))
    }
  ...
  prepareCartItemDetailsList(productDetails: any, cartItemsObject: any): Array<GuestCartItemDetails>{
    let cartItemDetailsList: GuestCartItemDetails[] = []
    for(let product in productDetails.hits.hit){
      cartItemDetailsList.push(new GuestCartItemDetails(

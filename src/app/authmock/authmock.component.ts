...
export class AuthmockComponent implements OnInit {
  ...
  confirmRegistration() {
    ...
    const promise = this._confirmRegistrationService.confirmRegistration(
  ...
  queryProducts() {
    this._productsService.get(this._query).subscribe(data => {
      console.log(data);
  ...
  getProductHierarchy() {
    this._productHierarchyService.get().subscribe(data => {
      console.log(data);
  ...
  getAddressDetails() {
    this._addressDetailsService
      .getAddressDetails(this.address_details_id)
      .subscribe(data => {
        console.log(data);
  ...
  deleteAddressDetails() {
    this._addressDetailsService
      .deleteAddressDetails(this.address_details_id)
      .subscribe(data => {
        console.log(data);
  ...
  getAddressDetailsListMock() {
    this._addressDetailsMockService.getAddressDetailsList().subscribe(data => {
      console.log(data);
      if (data['length'] > 0) {
  ...
  postAddressDetailsMock() {
    this._addressDetailsMockService
      .postAddressDetails(this.addressDetails)
      .subscribe(data => {
        console.log(data);
        if (data['insertId']) {
  ...
  deleteAddressDetailsMock() {
    this._addressDetailsMockService.deleteAddressDetails(1).subscribe(data => {
      console.log(data);
      this.mock_address_details_result =
  ...
  makeCartItem(): CartItem {
    ...
    this._cart_item_id =
      this._cart_item_id < 1 ? this._cart_item_id + 1 : this._cart_item_id;
  ...
  makeGuestCartItem(): GuestCartItem {
    ...
    this.guestCartProductId =
      this.guestCartProductId < 1
        ? this.guestCartProductId + 1
        : this.guestCartProductId;
  ...
  postGuestCartItemList() {
    this._guestCartService.getGuestCartItems().then((data: any) => {
      ...
      for (ele in data) {
        ...
        items.push(
          this.updateGuestCartItem(data[ele].guestCartItem.product_id)
        );
      }
      console.log('Data after change', items);
